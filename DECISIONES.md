...
un canal de alto volumen mostró un `mix_cuotas_pct` con un bucket en
...
**Evidencia preservada:** la salida real y truncada de esta corrida
(anonimizada) se guardó en
`corridas/falla_preservada/salida_truncada_original.json`, en vez de solo
describirla acá — para que quede el artefacto original de la falla, no
solo el relato.
...
## Iteración 8 — comparación real Sonnet vs. Haiku (cierre del requisito 5)

**Qué pasó:** al principio quedó documentado como limitación que no se
había corrido Haiku 4.5 por una restricción del artefacto de prueba (que
parecía fijo a Sonnet 4.6). Se intentó de todos modos, agregando un
selector de modelo al artefacto y pasando el string
`claude-haiku-4-5-20251001` como modelo — **funcionó**, sin ninguna
restricción real de la API.

**Qué se hizo:** se corrieron los mismos 3 meses (2025-11, 2026-03,
2026-06), sobre los mismos datos anonimizados, con Haiku 4.5. Resultado
completo en `corridas/comparacion_haiku_vs_sonnet/`:

- Costo real 68,1% menor que Sonnet (medido, no proyectado).
- 13 de 15 canales prioritarios coinciden exactamente con los de Sonnet
  (misma selección, misma categoría riesgo/oportunidad) en los 3 meses.
- Las 3 corridas de Haiku devolvieron JSON válido, respetando el esquema
  completo del contrato sin excepciones.

**Lección:** la limitación que documentamos en la iteración 7 ("no se
puede probar Haiku por el entorno") resultó ser una suposición no
verificada — asumí que la instrucción de usar un modelo fijo en el
artefacto era una restricción técnica dura, cuando en realidad era una
recomendación de default que se podía cambiar. La lección es la misma que
en la iteración 3: antes de aceptar una limitación como definitiva, vale
la pena intentar romperla una vez. Documentar honestamente "no lo probé"
en vez de inventar un resultado fue lo que permitió, más tarde, notar que
sí se podía probar y hacerlo — si hubiéramos simulado un resultado falso
de Haiku para "completar" el análisis económico, nunca hubiéramos hecho
esta prueba real.
